...
//!
//! Entry-points:
//!   deposit          ()               — adds caller's attached CSPR to nav_motes
...
pub extern "C" fn deposit() {
    // In production: caller transfers CSPR to the contract's purse.
    // Here we record the deposit amount as a named arg for clarity.
...
pub extern "C" fn execute_rebalance() {
    ...
    // parse weights and validate sum = 10000
    let weights: Vec<u32> = weights_bps
        .split(',')
        .filter_map(|s| s.trim().parse::<u32>().ok())
        .collect();
    if weights.iter().sum::<u32>() != 10_000 {
        runtime::revert(casper_types::ApiError::User(20)); // BadWeights
    }

    // record this rebalance
...
pub extern "C" fn call() {
    ...
    for (name, params) in &[
        ("deposit",          vec![Parameter::new("amount", CLType::U64)]),
        ("execute_rebalance",vec![
        ...
        ("get_nav",          vec![]),
        ("set_governance",   vec![Parameter::new("governance_hash", CLType::String)]),
    ] {
