...
pub extern "C" fn veto() {
    ...
    if state != STATE_PENDING {
        runtime::revert(casper_types::ApiError::User(32));
    }

    let window_ms   = read_u64(get_uref("veto_window_ms"));
    let now_ms: u64 = runtime::get_blocktime().into();
    if now_ms > proposed_at + window_ms {
        runtime::revert(casper_types::ApiError::User(33));
    }
...
pub extern "C" fn finalize() {
    ...
    let now_ms: u64 = runtime::get_blocktime().into();
    if now_ms <= proposed_at + window_ms {
        runtime::revert(casper_types::ApiError::User(34));
    }
...
pub extern "C" fn call() {
    ...
    for (name, params) in &[
        ("propose",        alloc::vec![Parameter::new("description", CLType::String)]),
        ("veto",           alloc::vec![Parameter::new("proposal_id", CLType::U64)]),
        ("finalize",       alloc::vec![Parameter::new("proposal_id", CLType::U64)]),
        ("get_proposal",   alloc::vec![Parameter::new("proposal_id", CLType::U64)]),
        ("proposal_count", alloc::vec![]),
    ] {
