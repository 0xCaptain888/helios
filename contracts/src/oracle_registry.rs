...
//!
//! Casper contract architecture (two-phase):
//!   Phase 1 — `call()` runs ONCE at install time:
//!              registers entry-point metadata + initialises named-key storage.
//!   Phase 2 — each subsequent invocation routes to a named export function
//!              (e.g. `register`, `credit_settlement`). These must exist as
//!              actual `#[no_mangle] pub extern "C" fn` symbols in the wasm —
//!              the EntryPoints metadata alone is not enough.
//!
...
use casper_types::{
    ...
    Key, Parameter, URef, contracts::NamedKeys,
};
...
// Oracle record serialised as pipe-delimited string:
// name|category|endpoint|price_motes|active(1/0)
...
// ── Phase 2: entry-point handler functions ────────────────────────────────────
// Every function listed in EntryPoints MUST have a matching #[no_mangle] export.
...
pub extern "C" fn register() {
    ...
    // idempotent: allow re-registration to update endpoint/price
...
pub extern "C" fn post_attestation() {
    ...
    let rep_uref = match runtime::get_key(&rep_key) {
        Some(k) => k.into_uref().unwrap_or_revert(),
        None    => runtime::revert(casper_types::ApiError::User(1)), // not registered
    };
    ...
    // store latest attestation value
...
pub extern "C" fn credit_settlement() {
    // Called by DataMarket after a successful sale.
    // Caller must be the registered market contract.
    let oracle: String = runtime::get_named_arg("oracle");

    // verify caller is the authorised market
    let market_uref = get_uref("market");
    let market: String = read_str(market_uref);
    let caller = runtime::get_caller().to_string();
    // on testnet the caller is the DataMarket contract entity hash
    // we allow admin to call too (for testing / manual correction)
    let admin_uref  = get_uref("admin");
    let admin: String = read_str(admin_uref);
    if caller != market && caller != admin {
        runtime::revert(casper_types::ApiError::User(2)); // unauthorized
    }

    let rep_key = format!("rep:{}", oracle);
    let rep_uref = match runtime::get_key(&rep_key) {
        Some(k) => k.into_uref().unwrap_or_revert(),
        None    => runtime::revert(casper_types::ApiError::User(3)), // oracle not found
    };
    let (sett, attest, acc, disp, _) = decode_rep(&read_str(rep_uref));
...
// ── Phase 1: install entry-point (runs once at deploy time) ───────────────────
...
pub extern "C" fn call() {
    ...
    for (name, params) in &[
        ...
        ("post_attestation", vec![
            Parameter::new("feed_key", CLType::String),
            Parameter::new("value",    CLType::String),
        ]),
        ("credit_settlement", vec![Parameter::new("oracle", CLType::String)]),
        ("score_attestation", vec![
            Parameter::new("oracle",   CLType::String),
            Parameter::new("accurate", CLType::Bool),
        ]),
        ("set_market",    vec![Parameter::new("market", CLType::String)]),
        ("get_oracle",    vec![Parameter::new("oracle", CLType::String)]),
        ("get_reputation",vec![Parameter::new("oracle", CLType::String)]),
    ] {
...
// ── Unit tests (host build, no wasm needed) ───────────────────────────────────
