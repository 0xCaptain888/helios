...
def _len_prefix(b: bytes) -> bytes: return _u32(len(b)) + b
def _str(s: str) -> bytes: return _len_prefix(s.encode())
...
def wait_for_deploy(deploy_hash: str, timeout: int = 300,
                    node: str = NODES[0]) -> dict:
    print(f"   waiting for {deploy_hash[:16]}…", end="", flush=True)
    deadline = time.time() + timeout
    while time.time() < deadline:
        try:
            r = _rpc("info_get_transaction",
...
def _header_hash(acct_hash: str, ts_ms: int, ttl_ms: int,
                 gas_price: int, body_h: str, chain: str) -> bytes:
    raw = (bytes.fromhex(acct_hash)
           + _u64(ts_ms) + _u64(ttl_ms) + _u64(gas_price)
...
def _contract_session_bytes(contract_hash: str, entry_point: str,
                             args: list) -> bytes:
    arg_bytes = b"".join(_named_arg(n, t, v) for n, t, v in args)
    return (b"\x01"
...
def _session_to_json(raw: bytes) -> dict:
    tag = raw[0]
    if tag == 0:
        return {"ModuleBytes": {"module_bytes": raw[5:].hex(), "args": []}}
    elif tag == 1:
        ep_len = struct.unpack("<I", raw[33:37])[0]
        ep = raw[37:37+ep_len].decode()
        return {"StoredContractByHash": {
            "hash": raw[1:33].hex(), "entry_point": ep, "args": []}}
    return {"ModuleBytes": {"module_bytes": raw.hex(), "args": []}}

def send_deploy(key: Ed25519PrivateKey, payment_motes: int,
                session_raw: bytes, node: str = NODES[0]) -> str:
    pub     = key_pub_bytes(key)
...
def install_wasm(key: Ed25519PrivateKey, wasm_path: str,
                 named_args: list | None = None,
                 payment: int = 400_000_000_000,
                 node: str = NODES[0]) -> str:
    wasm = Path(wasm_path).read_bytes()
    return send_deploy(key, payment,
                       _wasm_session_bytes(wasm, named_args or []), node)

def call_entry_point(key: Ed25519PrivateKey, contract_hash: str,
                     entry_point: str, named_args: list | None = None,
                     payment: int = 5_000_000_000,
                     node: str = NODES[0]) -> str:
    return send_deploy(key, payment,
                       _contract_session_bytes(contract_hash, entry_point,
                                               named_args or []), node)

# ── Arg helpers ───────────────────────────────────────────────────────────────

def arg_string(name: str, v: str):   return (name, _CL_STRING, _str(v))
def arg_u64(name: str, v: int):      return (name, _CL_U64, _u64(v))
def arg_u32(name: str, v: int):      return (name, _CL_U32, _u32(v))
def arg_u512(name: str, v: int):     return (name, _CL_U512, _u512(v))
def arg_bool(name: str, v: bool):    return (name, _CL_BOOL, bytes([1 if v else 0]))
...
def main() -> None:
    ...
    # ── install ───────────────────────────────────────────────────────────────
        ...
        dh = install_wasm(key, args.wasm, named_args=named,
                          payment=args.payment, node=args.node)
        print(f"Deploy   : {dh}")
    ...
    # ── call ──────────────────────────────────────────────────────────────────
        ...
        key   = load_key(args.key)
        named = _parse_args(args.args)
        dh = call_entry_point(key, args.contract, args.entry_point,
                               named, args.payment, args.node)
        print(f"Deploy   : {dh}")
        print(f"Explorer : {EXPLORER}/deploy/{dh}")
        if args.wait:
            wait_for_deploy(dh, node=args.node)
...
def _parse_args(raw: list[str]) -> list:
    ...
    for a in raw:
        ...
        if   typ == "string": result.append(arg_string(name, val))
        elif typ == "u64":    result.append(arg_u64(name, int(val)))
        elif typ == "u32":    result.append(arg_u32(name, int(val)))
        elif typ == "u512":   result.append(arg_u512(name, int(val)))
        elif typ == "bool":   result.append(arg_bool(name, val.lower() in ("1","true","yes")))
        else: raise ValueError(f"Unknown arg type '{typ}' in '{a}' — use string/u64/u32/u512/bool")
    return result
