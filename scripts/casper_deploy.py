...
def get_deploy_status(deploy_hash: str, node: str = NODES[0]) -> dict:
    return _rpc("info_get_transaction", {"transaction_hash": {"Deploy": deploy_hash}}, node)
...
def wait_for_deploy(deploy_hash: str, timeout: int = 180, node: str = NODES[0]) -> dict:
    ...
    while time.time() < deadline:
        try:
            ...
            exec_results = txn.get("execution_info", {}) or txn.get("execution_results", [])
            if exec_results:
                print(" ✓")
                return result
        except Exception:
...
def _make_deploy_hash(account_hash: str, timestamp_ms: int, ttl_ms: int,
                      gas_price: int, body_hash: str, dependencies: list,
                      chain_name: str) -> bytes:
...
def _standard_payment(amount_motes: int) -> bytes:
    ...
    return b"\x00" + _bytes_with_u32_len(module_bytes) + _bytes_with_u32_len(args_serialized)

def _wasm_session(wasm_bytes: bytes, named_args: list[tuple[str, bytes, bytes]]) -> bytes:
    ...
    return b"\x00" + _bytes_with_u32_len(wasm_bytes) + _bytes_with_u32_len(args_serialized)

def _stored_contract_session(contract_hash: str, entry_point: str,
                              named_args: list[tuple[str, bytes, bytes]]) -> bytes:
...
def build_and_send_deploy(
    ...
    deploy_hash_bytes = _make_deploy_hash(account_hash, now_ms, ttl_ms, gas_price, body_h, [], CHAIN)
...
def _session_json(session_bytes: bytes) -> dict:
    ...
    if tag == b"\x00":
        ...
        return {"StoredContractByHash": {"hash": hash_hex, "entry_point": ep, "args": []}}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Helios Casper deploy tool (pure Python)")
    ...
    if args.cmd == "keygen":
        ...
        deploy_hash = install_wasm(key, args.wasm, payment=args.payment, node=args.node)
        ...
        for a in args.args:
            ...
            if typ == "string": named.append(arg_string(name, val))
            elif typ == "u64":  named.append(arg_u64(name, int(val)))
            elif typ == "u32":  named.append(arg_u32(name, int(val)))
            elif typ == "u512": named.append(arg_u512(name, int(val)))
